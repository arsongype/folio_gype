      ...
      <div className="flex items-center h-full justify-between w-[90%] mx-auto">
        ...
        <ul className='hidden lg:flex space-x-10'>
          {navLinks.map((link) => {
            return (
              <li key={link.url}>
                <Link className='text-green-200 hover:text-cyan-300 font-medium transition-colors duration-300' href={link.url}>{link.label}</Link>
              </li>
            )
          })}
        </ul>
        ...
        {/* <div className='hidden lg:block'>
          <LinkButton href='/documents/cv.pdf' text='Download CV' download icon={LuDownload} iconPosition='left' />
        </div> */}
